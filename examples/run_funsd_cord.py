...
def main():
    ...
    def reading_order_sort(words, bboxes, labels):
        """
        Sắp xếp các từ theo thứ tự đọc tự nhiên (từ trên xuống, trái sang phải)
        mô phỏng thuật toán XY-Cut.
        """
        if not words:
            return words, bboxes, labels
            
        # Zip cùng với index gốc để đảm bảo tính Stable Sort:
        # Các từ có CÙNG bounding box (thuộc cùng 1 segment) sẽ không bao giờ bị xé lẻ.
        items = list(zip(words, bboxes, labels, range(len(words))))
        
        # Bước 1: Sort nháp theo tọa độ Y_min
        items.sort(key=lambda x: (x[1][1], x[1][0], x[3])) 
        
        # Bước 2: Gom nhóm thành các dòng (Lines)
        lines = []
        current_line = []
        for item in items:
            box = item[1]
            if not current_line:
                ...
                # Tính Y_min, Y_max trung bình của dòng hiện tại
                line_y_min = sum(itm[1][1] for itm in current_line) / len(current_line)
                line_y_max = sum(itm[1][3] for itm in current_line) / len(current_line)
                
                box_y_min, box_y_max = box[1], box[3]
                
                # Tính độ chồng lấn (overlap) theo trục Y
                overlap_min = max(line_y_min, box_y_min)
                overlap_max = min(line_y_max, box_y_max)
                overlap_h = max(0, overlap_max - overlap_min)
                
                box_h = box_y_max - box_y_min
                line_h = line_y_max - line_y_min
                
                # Nếu chồng lấn dọc > 30% chiều cao -> chung một dòng
                if overlap_h > 0.3 * min(box_h, line_h + 1e-4):
                    current_line.append(item)
                else:
        ...
        # Bước 3: Trong mỗi dòng, sort theo tọa độ X_min
        sorted_items = []
        for line in lines:
            line.sort(key=lambda x: (x[1][0], x[3])) 
            sorted_items.extend(line)
            
        s_words = [item[0] for item in sorted_items]
        s_bboxes = [item[1] for item in sorted_items]
        s_labels = [item[2] for item in sorted_items]
        
        return s_words, s_bboxes, s_labels

    # Tokenize all texts and align the labels with them.
    def tokenize_and_align_labels(examples, augmentation=False):
        
        # --- THÊM MỚI: Kích hoạt XY-Cut Reading Order ---
        batch_size = len(examples[text_column_name])
        for i in range(batch_size):
            w, b, l = reading_order_sort(
                examples[text_column_name][i], 
                examples["bboxes"][i], 
                examples[label_column_name][i]
            )
            examples[text_column_name][i] = w
            examples["bboxes"][i] = b
            examples[label_column_name][i] = l
        # ------------------------------------------------
