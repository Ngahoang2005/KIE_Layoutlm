...
class DataTrainingArguments:
    ...
    use_segment_residual_head: bool = field(
        default=False,
        metadata={
            "help": "Use the residual-fusion segment model. This keeps the original token representation and "
            "adds a learned residual segment-context contribution instead of hard segment broadcast."
        },
    )
...
def main():
    ...
    if data_args.use_segment_head and data_args.use_segment_residual_head:
        raise ValueError(
            "Use only one of --use_segment_head or --use_segment_residual_head."
        )
    ...
    if getattr(data_args, "use_segment_residual_head", False):
        # NEW residual segment model: preserve token-level hidden states and
        # inject contextualized segment information through residual fusion.
        from layoutlmft.models.layoutlmv3.modeling_layoutlmv3_segment_residual import (
        ...
        # Existing hard-broadcast segment model. Kept unchanged for backward
        # compatibility with the original experiments/checkpoints.
    ...
    # Runtime sanity check for the NEW residual architecture.
    if getattr(data_args, "use_segment_residual_head", False):
        if not hasattr(model, "segment_fusion_alpha"):
            raise RuntimeError(
                "Residual segment model is not loaded: missing `segment_fusion_alpha`. "
                "Check modeling_layoutlmv3_segment_residual.py."
            )
        if hasattr(model, "is_first_token_embedding"):
            raise RuntimeError(
                "Old segment architecture detected. The residual experiment must not use "
                "`is_first_token_embedding`."
            )
        logger.info(
            "Using RESIDUAL segment fusion: z_i = h_i + alpha * W_s(s'_k), "
            f"initial alpha={model.segment_fusion_alpha.item():.6f}"
        )
    ...
    def tokenize_and_align_labels(examples, augmentation=False):
        ...
        for batch_index in range(len(tokenized_inputs["input_ids"])):
            ...
            # needed.
            # Computed only for either segment-aware model. The vanilla
            # LayoutLMv3ForTokenClassification path never receives `seg_id`.
            word_seg_id = None
            if (
                getattr(data_args, "use_segment_head", False)
                or getattr(data_args, "use_segment_residual_head", False)
            ):
                word_seg_id = []
                seg_counter = -1
    ...
    class CustomTrainer(Trainer):
        def create_optimizer(self):
            if self.optimizer is None:
                ...
                # Nhóm 2: Các tham số mới (segment context, classifier,
                # residual fusion alpha/projection, context gate, etc.)
